...
fn process_prompt_spreadsheet(
    ...
    for (row_index, row) in rows.iter().enumerate() {
        ...
        let mut non_empty_identifiers = Vec::new();

        for &index in &identifier_indexes {
            let value = row.get(index).cloned().unwrap_or_default();
            if !value.trim().is_empty() {
                non_empty_identifiers.push(value.clone());
            }
            identifier_values.push(value);
        }

        let identifier_label = if non_empty_identifiers.is_empty() {
            format!("Row {}", row_number)
        } else {
            non_empty_identifiers.join(" – ")
        };

        let warning_label = if non_empty_identifiers.is_empty() {
            format!("row {}", row_number)
        } else {
            format!("row {} ({})", row_number, identifier_label)
        };
    ...
    let mut headers: Vec<String> = identifier_columns
        .iter()
